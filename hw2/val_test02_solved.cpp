...
//
//  Bug:
//    Initializer loop did not initialize all entries in x which were used in the last loop
//    for printing. Valgrind would not care about uninitialized values as long as they don't
//    affect program output (as in the loop where we set x[i] = x[i] * 2). But printing
//    uninitialized values raises valgrind errors. I fixed it by changing the terminating
//    condition in the initializer loop to i < 10 so that all entries in x are initialized.
{
  int i;
  int *x;
...
//
  // for ( i = 0; i < 5; i++ )
  for ( i = 0; i < 10; i++ ) // Bug here: did not initialize all entries in x
  {
    x[i] = i;
  }
//
